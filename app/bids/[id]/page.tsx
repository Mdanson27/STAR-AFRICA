...
export default async function BidDetailPage({params}:{params:Promise<{id:string}>}) {
  const {id}=await params; const session=await requireSession(`/bids/${id}`); let bid=demoBids.find((item)=>item.id===id);
  ...
  if(!bid)notFound();
  return <AppShell active="Bids & tenders" user={{name:session.name,role:session.role}}><BidWorkspace bid={bid} permissions={[...session.permissions]} currentUser={session.name}/></AppShell>;
}
